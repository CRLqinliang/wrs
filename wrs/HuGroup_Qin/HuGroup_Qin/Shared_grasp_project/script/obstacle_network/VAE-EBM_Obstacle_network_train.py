...
class VoxelBetaVAE(nn.Module):
    def __init__(self, in_channels=1, latent_dim=128, beta=4.0):
        super(VoxelBetaVAE, self).__init__()
...
class VoxelGraspDataset(Dataset):
    def __init__(self, data_path, grasp_info_path=None, transform=None):
        """
        加载从ObstacleGraspNetwork_data_collection.py收集的数据
        
        参数:
            data_path: .npz文件路径，包含voxel_data和grasp_ids
        ...
        print("正在加载数据集...")
        # 加载.npz文件中的体素数据和可行grasp_id
        data = np.load(data_path, allow_pickle=True)
        self.voxel_data = data['voxel_data']
        self.grasp_ids = data['grasp_ids']
        
        # 如果数据集中包含标签，直接加载
        self.labels_data = data.get('labels', None)
    ...
    def _prepare_data(self):
        """预处理数据，为每个体素场景和抓取姿态生成样本对"""
        # 预先计算总样本数
        total_samples = 0
        for idx in range(len(self.voxel_data)):
            available_gids = self.grasp_ids[idx]
            if available_gids is not None:
                total_samples += len(available_gids)
        
        # 预分配数组
        self.features = []
        self.labels = np.zeros(total_samples, dtype=np.float32)
        ...
        for idx in tqdm(range(len(self.voxel_data)), desc="处理样本"):
            ...
            # 如果存在有效抓取
            if available_gids is not None and len(available_gids) > 0:
                for i, gid in enumerate(available_gids):
                    if self.grasp_info and gid in self.processed_grasps:
                        grasp_pose = self.processed_grasps[gid]
                        
                        # 添加特征
                        self.features.append((voxel_tensor, torch.FloatTensor(grasp_pose)))
                        
                        # 设置标签 - 借鉴EBM_grasp_network_train.py的方式
                        if self.labels_data is not None and idx < len(self.labels_data):
                            # 如果有明确的标签数据，使用它
                            label = self.labels_data[idx].get(str(gid), 0)  # 默认为0如果找不到
                        else:
                            # 否则，假设所有available_gids都是正样本
                            label = 1
                        
                        self.labels[current_idx] = label
                        current_idx += 1
            
            # 清理临时变量
            del voxel_tensor
        
        # 确保labels长度与features匹配
        self.labels = self.labels[:current_idx]
...
def train_model(vae, energy_model, train_loader, val_loader, criterion_vae, criterion_energy, optimizer_vae, 
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Train - VAE Loss: {avg_train_vae_loss:.4f}, Energy Loss: {avg_train_energy_loss:.4f}, F1: {train_metrics['f1']:.4f}, Energy Gap: {train_metrics['energy_gap']:.4f}")
        print(f"Val - VAE Loss: {avg_val_vae_loss:.4f}, Energy Loss: {avg_val_energy_loss:.4f}, F1: {val_metrics['f1']:.4f}, Energy Gap: {val_metrics['energy_gap']:.4f}")
        
        # 早停与模型保存
        current_metric = val_metrics['energy_gap']
...
def parse_args():
    ...
    parser.add_argument('--hidden_dims', type=int, nargs='+', default=[256, 128, 64], help='能量模型的隐藏层维度')
    ...
    parser.add_argument('--energy_margin', type=float, default=1.0, help='能量损失间隔')
    ...
    parser.add_argument('--load_model', type=str, default=None, help='加载已有模型路径')
...
def main():
    ...
    # 创建数据集
    full_dataset = VoxelGraspDataset(
    ...
    # 使用自定义的SubsetWithLabels类来保留标签信息
    class SubsetWithLabels(torch.utils.data.Subset):
        def __init__(self, dataset, indices):
            super().__init__(dataset, indices)
            self.labels_tensor = dataset.labels_tensor[indices]
    ...
    # 创建模型
    vae = VoxelBetaVAE(in_channels=1, latent_dim=args.latent_dim, beta=args.beta).to(device)
    ...
    if args.load_model:
        ...
        vae.load_state_dict(checkpoint['vae_state_dict'])
        energy_model.load_state_dict(checkpoint['energy_model_state_dict'])
        print(f"模型加载完成，最佳指标: {checkpoint.get('best_metric', 'N/A')}")
