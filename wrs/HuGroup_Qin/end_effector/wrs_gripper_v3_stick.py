...
class WRSGripper3(gpi.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3),
        ...
        # anchor
        self.jlc.anchor.lnk_list[0].cmodel = mcm.CollisionModel(os.path.join(current_file_dir, "meshes", "stick_gripper_hand.stl"),
                                                                name="wg_v3_base",
        ...
        self.jlc.jnts[0].loc_pos = np.array([0, .0, .057])
        self.jlc.jnts[0].loc_motion_ax = rm.const.x_ax
        self.jlc.jnts[0].lnk.loc_rotmat = rm.rotmat_from_euler(0, 0, np.pi)
        self.jlc.jnts[0].lnk.cmodel = mcm.CollisionModel(os.path.join(current_file_dir, "meshes", "stick_gripper_finger.stl"),
                                                         name="wg_v3_finger1",
                                                         cdmesh_type=self.cdmesh_type,
                                                         cdprim_type=mcm.const.CDPrimType.USER_DEFINED,
        ...
        self.jlc.jnts[1].change_type(rkjlc.const.JntType.PRISMATIC, motion_range=np.array([0.0, self.jaw_range[1] / 2]))
        self.jlc.jnts[1].loc_pos = np.array([-.0, .0, .0])
        self.jlc.jnts[1].loc_motion_ax = -rm.const.x_ax
        self.jlc.jnts[1].lnk.cmodel = mcm.CollisionModel(os.path.join(current_file_dir, "meshes", "stick_gripper_finger.stl"),
                                                         name="wg_v3_finger2",
                                                         cdmesh_type=self.cdmesh_type,
                                                         cdprim_type=mcm.const.CDPrimType.USER_DEFINED,
                                                         userdef_cdprim_fn=self._finger_cdprim, ex_radius=.005)
...
if __name__ == '__main__':
    ...
    gripper.gen_meshmodel(rgb=np.array([1, 0, 0]), toggle_tcp_frame=True, toggle_cdprim=False, toggle_cdmesh=False).attach_to(base)
