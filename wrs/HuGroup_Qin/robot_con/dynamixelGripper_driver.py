"""
A high-level API for the dynamixel based gripper for the sdk_wrapper.py.
Author: Liang Qin
Reference: Chen Hao (chen960216@gmail.com), 20221015, osaka
Update Notes:
...
import numpy as np
from wrs.HuGroup_Qin.driver.motor_driver.sdk_wrapper import DynamixelMotor

# get the path of the current file
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
CALIBRATION_DATA_FILE_NAME = os.path.join(THIS_DIR, "dynamixel_gripper_calibration_data.json")
...
class DynamixelGripper(object):
    ...
    def calibrate(self, close_gripper_direction: Literal[-1, 1] = 1, speed: int = 100):
        ...
        [self._motor_x.set_dxl_goal_current(170, motor_id) for motor_id in self._motor_ids]
        time.sleep(.1)
...
if __name__ == "__main__":
    try:
        gp = DynamixelGripper(device='COM3', motor_ids=[1], baudrate=57600,
                              op_mode=5, gripper_limit=(0, 0.18),
