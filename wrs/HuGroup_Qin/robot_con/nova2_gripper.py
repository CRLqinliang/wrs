"""
WRS control interface for Nova 2 of HuGroup
Author: Liang Qin (qinl.drlrobot@gmail.com)
Reference: Hao Chen <chen960216@gmail.com> 20230113, osaka
...
from wrs.HuGroup_Qin.driver.robot_driver.dobot_api import Dobot
...
class Nova2Gripper(object):
    ...
    def orientation_arm2wrs(arr: np.ndarray) -> np.ndarray:
        ...
        """
        return rm.rotmat_from_euler(*Nova2Gripper.angle_arm2wrs(arr))
    ...
    def __init__(self, ip: str = "192.168.5.1", has_gripper=False, init_enable_rbt=True):
        ...
        if has_gripper:
            self.gripper_x: DynamixelGripper = DynamixelGripper(device='COM3', motor_ids=[1], baudrate=57600, op_mode=5,
                                                          gripper_limit=(0, 0.18),
                                                          motor_max_current=170)
        else:
            self.gripper_x: DynamixelGripper = None
...
if __name__ == "__main__":
    ...
    # lft_arm : 192.169.5.100

    lft_arm = Nova2Gripper(ip="192.168.5.100", has_gripper=False, init_enable_rbt=True)

    current_jnv = np.array([-1.2034, -18.1839, 136.3675, -28.1078, -90.09, -330.2170]) * np.pi / 180
    target_jnv6 = 540 * np.pi / 180
    target_jnv = current_jnv + np.array([0, 0, 0, 0, 0, target_jnv6])
    n = 5
    path = np.linspace(current_jnv, target_jnv, n)
    lft_arm.move_jntspace_path(path, control_frequency=1 / 125)
