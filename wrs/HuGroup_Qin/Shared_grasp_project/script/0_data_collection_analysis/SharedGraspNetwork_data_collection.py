...
SAVE_PREFIX = "SharedGraspNetwork_bottle_experiment_data_final"
OBJECT_MESH_PATH = r'E:\Qin\wrs\wrs\HuGroup_Qin\objects\meshes\bottle.stl'
...
def obj_setup(name, pos, rotmat, rgb=None, alpha=None):
    ...
    obj_cmodel = mcm.CollisionModel(name=name, rgb=rgb, alpha=alpha, initor=OBJECT_MESH_PATH)
    obj_cmodel.pos = pos
...
def show_common_grasp(obj_init_pos, obj_init_rotmat, obj_goal_pos, obj_goal_rotmat,
    ...
    init_obj = obj_setup(name="init_obj", pos=obj_init_pos, rotmat=obj_init_rotmat, alpha=0.5)
    goal_obj = obj_setup(name="goal_obj", pos=obj_goal_pos, rotmat=obj_goal_rotmat, alpha=0.5)
    ...
    # init_obj.show_local_frame()
    init_obj.attach_to(base)
    # goal_obj.show_local_frame()
...
def show_grasp(robot_env, obj_cmodel, obj_pos, obj_rotmat, grasp_id, object_feasible_grasps, gripper):
    ...
    for index in grasp_id:
        ...
        gripper.gen_meshmodel(toggle_cdprim=False, alpha=0.5).attach_to(base)
        ...
        #             if not robot_env.is_collided(obstacle_list = [obj_cmodel]):
...
if __name__ == '__main__':
    ...
    obj_cmodel = mcm.CollisionModel(initor=OBJECT_MESH_PATH)
        ...
        with tqdm(total=TOTAL_ITERATIONS, desc=f"Processing grasp_id {grasp_id}") as pbar:
            ...
            for sample_idx in range(TOTAL_ITERATIONS):
                ...
                init_obj = obj_setup(name="init_obj", pos=init_pos_for_feasible,
                                     rotmat=init_rotmat_for_feasible, alpha=0.3)
                ...
                if common_id:
                    # show_grasp(robot, init_obj, init_pos_for_feasible, init_rotmat_for_feasible,
                    #            init_available_gids_robot_table, object_feasible_grasps, gripper)
                    # show_grasp(robot, goal_obj, goal_pos_for_feasible, goal_rotmat_for_feasible,
                    #            goal_available_gids_robot_table, object_feasible_grasps, gripper)
                    show_grasp(robot, goal_obj, goal_pos_for_feasible, goal_rotmat_for_feasible,
                               common_id, object_feasible_grasps, gripper)
                    show_common_grasp(
