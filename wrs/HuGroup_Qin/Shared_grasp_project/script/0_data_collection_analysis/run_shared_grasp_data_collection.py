...
def run_shared_grasp_data_collection(grasp_ids=[109], total_iterations=int(1e4), save_batch_size=1000):
    ...
    """
    script_path = r"E:\Qin\wrs\wrs\HuGroup_Qin\Shared_grasp_project\script\0_data_collection_analysis\SharedGraspNetwork_data_collection.py"
    script_name = "SharedGraspNetwork_data_collection.py"  # 脚本名称
...
def parse_args():
    ...
    parser.add_argument('--runs', type=int, default=50,  help='运行次数')  # 0.3M  - Shared grasp 0.3M , feasible grasp 0.6M.
    parser.add_argument('--grasp_ids', type=int, nargs='+', default=[195], help='要处理的抓取ID列表')
