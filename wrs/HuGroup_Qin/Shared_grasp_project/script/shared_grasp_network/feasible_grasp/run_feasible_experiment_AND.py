...
def run_bc_and_evaluation(hidden_dims, num_layers, dropout_rate, batch_size, 
                         dataset_type, data_id, train_split, use_quaternion, 
                         use_stable_label, grasp_type, state_type, model_mode, model_ratios, data_ratio):
    """运行BC模型的AND评估
    ...
    script_path = os.path.join(current_dir, f'grasp_network_AND_evaluation.py')
    ...
    if model_mode == 'standard':
        ...
        # 构建模型路径
        init_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{init_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_seed22'
        goal_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{goal_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_seed22'
        ...
        # wandb配置
        and_exp_name = f'bc_and_{dataset_type}_{data_id}_i{init_ratio}_g{goal_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gt{grasp_type}_st{state_type}'
        cmd.extend(['--wandb_project', 'bc_grasp_AND_evaluation'])
        ...
        # 构建模型路径 - Robot
        robot_init_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{robot_init_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gtrobot_stinit_seed22'
        ...
        # 构建模型路径 - Table
        table_init_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{table_init_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gttable_stinit_seed22'
        ...
        # 构建模型路径 - Robot
        robot_goal_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{robot_goal_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gtrobot_stinit_seed22'
        ...
        # 构建模型路径 - Table
        table_goal_exp_name = f'BC_feasible_{dataset_type}_{data_id}_h{len(hidden_dims)}_b{batch_size}_lr0.001_r{table_goal_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gttable_stinit_seed22'
        ...
        # wandb配置
        and_exp_name = f'bc_feasible_and_rt_{dataset_type}_{data_id}_ri{robot_init_ratio}_ti{table_init_ratio}_rg{robot_goal_ratio}_tg{table_goal_ratio}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_gt{"robot-table"}_st{state_type}'
        cmd.extend(['--wandb_project', 'bc_grasp_feasible_AND_RT_evaluation'])
        cmd.extend(['--wandb_name', and_exp_name])
        
        print(f"\n开始 BC AND 扩展评估 (Robot+Table)...")
...
def main():
    ...
    grasp_types = ['robot_table'] # 'robot_table' or 'robot' or 'table'
    state_types = ['both'] # 'init' or 'goal' or 'both'
