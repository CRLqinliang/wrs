...
class SharedGraspDataset(Dataset):
    def __init__(self, data, grasp_pickle_file, num_classes=None, use_quaternion=False, use_stable_label=True):
        """
        Args:
    ...
    def prepare_data(self):
        ...
        if self.num_classes is None:
            # 找出所有数据中的common_id列表中的最大值
            max_common_id = 0
            for item in self.data:
                if item[-1] and len(item[-1]) > 0:  # 确保common_id存在且非空
                    max_common_id = max(max_common_id, max(item[-1]))
            self.num_classes = max_common_id + 1
        
        # 创建多标签向量
        all_labels = np.zeros((total_samples, self.num_classes), dtype=np.float32)
        ...
        for item in self.data:
            ...
            # 设置多标签
            if item[-1]:  # common_id
                for label_idx in item[-1]:
                    all_labels[current_idx:end_idx, label_idx] = 1
...
class GraspNetwork(nn.Module):
    def __init__(self, input_dim, num_classes=1, hidden_dims=None, num_layers=3, dropout_rate=0.1):
        super().__init__()
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler,
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loop:
            ...
            loss = criterion(outputs, labels)  # 多标签损失
        ...
        with torch.no_grad():
            for val_inputs, val_labels in val_loop:
                ...
                current_val_loss = criterion(val_outputs, val_labels).item()
        ...
        wandb.log({
            ...
            "train/macro_precision": train_metrics['macro_precision'],
            "train/macro_recall": train_metrics['macro_recall'],
            "train/macro_f1": train_metrics['macro_f1'],
            "train/micro_precision": train_metrics['micro_precision'],
            "train/micro_recall": train_metrics['micro_recall'],
            "train/micro_f1": train_metrics['micro_f1'],
        ...
        print(f"Train Loss: {avg_train_loss:.4f}, Micro F1:{train_metrics['micro_f1']:.4f}, Micro Precision:{train_metrics['micro_precision']:.4f},"
              f" Micro Recall: {train_metrics['micro_recall']:.4f}")
        print(f"Val Loss: {avg_val_loss:.4f}, Micro F1:{val_metrics['micro_f1']:.4f}, Micro Precision:{val_metrics['micro_precision']:.4f},"
              f" Micro Recall: {val_metrics['micro_recall']:.4f}")
        print(f"Optimal Threshold - Train: {train_metrics['optimal_threshold']:.4f}, Val: {val_metrics['optimal_threshold']:.4f}")

        # 验证阶段后的早停检查
        if val_metrics['micro_f1'] > best_val_metric:  # 使用micro F1分数作为早停标准
            best_val_metric = val_metrics['micro_f1']
            ...
            print(f"\n保存最佳模型 - 验证集Micro F1: {best_val_metric:.4f}, 最佳阈值: {best_threshold:.4f}")
            torch.save(save_info, save_path)
            
            # 额外分析：使用验证集阈值在训练集上的表现
            train_with_val_threshold = calculate_metrics_with_fixed_threshold(
                train_outputs, train_labels, best_threshold)
            print(f"使用验证集阈值在训练集上的Micro F1: {train_with_val_threshold['micro_f1']:.4f}")
            
            # 记录到wandb
            wandb.log({
                "val/best_val_micro_f1": best_val_metric,
                "val/best_threshold": best_threshold,
                "train/train_micro_f1_with_val_threshold": train_with_val_threshold['micro_f1']
            })
        else:
            patience_counter += 1
            
        if patience_counter >= early_stop_patience:
            print(f"\n早停: 验证集Micro F1分数 {early_stop_patience} 个epoch没有改善")
            break
    ...
    print(f"最佳验证集Micro F1: {best_model_info['best_val_metric']:.4f}")
...
def create_datasets(raw_data, indices, args):
    # 设置num_classes等于data_id
    num_classes = args.data_id
...
def setup_training(args):
    ...
    # 使用data_id作为num_classes
    num_classes = args.data_id
    print(f"使用 data_id={num_classes} 作为输出类别数量")
...
def parse_args():
    ...
    # 如果未指定data_id，尝试从grasp_data_path中提取
    if args.data_id is None:
        import re
        match = re.search(r'(\d+)\.pickle$', args.grasp_data_path)
        if match:
            args.data_id = int(match.group(1))
            print(f"从文件路径提取的data_id: {args.data_id}")
        else:
            raise ValueError("无法从grasp_data_path提取data_id，请手动指定--data_id参数")
