...
class SharedGraspEnergyDataset(Dataset):
    def __init__(self, data, grasp_pickle_file, use_quaternion=False, use_stable_label=True, 
        ...
        ee_values = np.array([grasp.ee_values[0] for grasp in grasp_candidates])
        ...
        grasp_poses = np.array([
            np.concatenate([
                ...
                [(grasp.ee_values[0] - ee_min) / (ee_max - ee_min)]  # 归一化ee_values
    ...
    def prepare_data(self):
        ...
        # 计算特征维度
        if self.use_quaternion:
            pose_dim = 8  # pos(3) + quaternion(4) + normalized_width(1)
        else:
            pose_dim = 3  # pos(2) + normalized_rz(1)
            
        grasp_pose_dim = 8  # 抓取位姿始终使用8维表示
...
class SharedGraspFinalEnergyDataset(Dataset):
    def __init__(self, data, grasp_pickle_files, use_quaternion=False, use_stable_label=True, 
                 grasp_type='robot_table', state_type='both'):
        """
        Args:
            data: 数据列表
            grasp_pickle_files: 逗号分隔的抓取候选文件路径字符串
            use_quaternion: 是否使用四元数表示旋转。如果为False，则使用简化的(x,y,rz)表示
            use_stable_label: 是否使用stable_label作为特征。注意：当use_quaternion=False时必须为True
            grasp_type: 使用哪种抓取集合，可选 'robot_table', 'table', 'robot'
            state_type: 处理哪种状态，可选 'init', 'goal', 'both'
        ...
        self.grasp_files_list = grasp_pickle_files[0].split(',')
        
        # TODO 只取前180个抓取候选
        for grasp_file in self.grasp_files_list:
            with open(grasp_file, 'rb') as f:
                grasp_candidates = pickle.load(f)
                # grasp_candidates = grasp_candidates._grasp_list[:180] # 只取前180个抓取候选
                all_grasp_candidates.extend(grasp_candidates)  
                print(f"从 {os.path.basename(grasp_file)} 加载了 {len(grasp_candidates)} 个抓取候选")
                
        # 首先收集所有ee_values用于计算最大最小值
        ee_values = np.array([grasp.ee_values for grasp in all_grasp_candidates])
        ee_min = np.min(ee_values)
        ee_max = np.max(ee_values)
        ...
        grasp_poses = np.array([
            np.concatenate([
                np.array(grasp.ac_pos, dtype=np.float32).flatten(),
                R.from_matrix(grasp.ac_rotmat).as_quat()
                # [(grasp.ee_values - ee_min) / (ee_max - ee_min)]  # 归一化ee_values
    ...
    def prepare_data(self):
        ...
        for item in self.data:
            ...
            if grasp_indices is not None:
                # 总样本数 = 抓取候选总数
                sample_count = len(self.grasp_poses)
                # sample_count = 180 * len(self.grasp_files_list) # 180个抓取候选
                total_samples += sample_count
        ...
        grasp_pose_dim = 7  # pos(3) + quaternion(4)
        ...
        for item_idx, item in enumerate(self.data):
            ...
            # 为该物体状态创建所有抓取的样本
            n_samples = len(self.grasp_poses)
            # n_samples = 180 * len(self.grasp_files_list)
            end_idx = current_idx + n_samples
...
def load_raw_data(data_paths, ratio=None, data_range=None):
...
def create_data_loaders(train_dataset, val_dataset, test_dataset, args):
    """创建数据加载器"""
    train_sampler = BalancedBatchSampler(train_dataset, args.batch_size) # Loss 里面有对比损失；
...
def parse_args():
    ...
    #                    help='数据文件路径列表')
    # parser.add_argument('--grasp_data_paths', nargs='+', type=str,
    #                    help='抓取数据文件路径列表')
    
    parser.add_argument('--data_paths', nargs='+', type=str, default=["E:\Qin\wrs\wrs\HuGroup_Qin\Shared_grasp_project\grasps\Bottle\SharedGraspNetwork_bottle_experiment_data_352.pickle"],
                       help='数据文件路径列表')
    parser.add_argument('--grasp_data_paths', nargs='+', type=str, default=["E:\Qin\wrs\wrs\HuGroup_Qin\Shared_grasp_project\grasps\Bottle\Bottle_grasp_352.pickle"],
                       help='抓取数据文件路径列表')
    ...
    parser.add_argument('--data_range', type=int, default=75000, help='数据采样个数')
