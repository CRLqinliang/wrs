...
def run_ebm_experiment(hidden_dims, num_layers, dropout_rate, batch_size, 
    ...
    """运行单个EBM实验
    Args:
        objects_with_ids: 物体和对应ID的字典，例如 {'Bottle': {'id': 352, 'dataset_type': 'SharedGraspNetwork_bottle_experiment_data_final'}}
    """
    # 获取当前文件的目录路径
    ...
    exp_name = f'ebm_{objects_str}_h{len(hidden_dims)}_b{batch_size}_lr{learning_rate}_t{temperature}_r{data_range}_s{train_split}_q{int(use_quaternion)}_sl{int(use_stable_label)}_g{grasp_type}_st{state_type}'
    ...
    print(f"\n开始训练多对象 EBM 网络...")
    print(f"训练对象: {', '.join(objects_with_ids.keys())}")
...
def main():
    ...
    object_info = {
        ...
        # 'Bottle': {
        #     'id': 57,
        #     'dataset_type': 'SharedGraspNetwork_bottle_experiment_data'
        # },
        'Bunny': {
            'id': 252,
